...
const Login = () => {
    ...
    const blockFrequentClick = useRef(true);
    ...
    const {
        ...
        getFirstName,
        showSendMailPage, setShowSendMailPage } = useContext(AppContext);
    ...
    const handelRegistration = async () => {
        if (blockFrequentClick.current == false) {
            console.log('block');
            
            return;
        }
        blockFrequentClick.current = false;
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (response.data.sendMailStatus.success) {
                    userDetails.current = response.data.user;
                    // console.log(userDetails);
                    toast.success(response.data.sendMailStatus.message);
                    setShowSendMailPage(true);
                } else {
                    toast.warning(response.data.sendMailStatus.message);
                }
        ...
        blockFrequentClick.current = true;
    };
    ...
    return (
        <>
            <div className='h-screen w-full fixed left-0 top-0 z-[11] backdrop-blur-[2px] bg-[#0000008b]
                ...
                {
                    showSendMailPage ?
                        <SendMailPage userDetails={userDetails.current} />
