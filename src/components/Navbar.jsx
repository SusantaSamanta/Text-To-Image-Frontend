...
const Navbar = () => {
    ...
    return (
        <>
            <div id='navbarStyle' className='w-full py-4 lg:px-32 border-0  flex justify-between fixed left-0 top-0 bg-[#ffffffb8] backdrop-blur-sm z-10'>
                <Link to='/' className='border-0 flex items-center w-auto'>
                    <img className='border-0 lg:scale-[1.3]' src='../app_logo.png' />
                </Link>
                <div className='border-0 w-auto flex flex-col justify-center items-center font-semibold '>
                    {
                        isLogin ?

                            <div className='border-0 w-auto flex items-center gap-4 text-gray-700'>
                                <Link to='/generate' className=' border-b-2 border-transparent hover:border-black transition duration-200'>Generate</Link>
                                <Link to='/generate' className='px-6 py-2 bg-blue-400 hover:bg-blue-500 shadow-[2px_2px_6px_#4a4a4a72] text-white rounded-2xl'>Credits left: 10</Link>
                                <Link to='/profile' className='flex items-center'>
                                    <p>Hi, Susanta</p>
                                    <div className='ml-2 h-10 w-10 flex items-center justify-center shadow-[1px_3px_10px_#4a4a4a72] bg-blue-500 text-white rounded-full border-2 relative group'>
                            ...
                            <div className='border-0 flex items-center gap-4 text-gray-700'>
                                <Link to='/generate' className=' border-b-2 border-transparent hover:border-gray-500 transition duration-300'>Generate</Link>
                                <Link to='/login' className='px-6 py-2 bg-black hover:bg-blue-900 text-white rounded-2xl transition duration-300'>Login</Link>
                            </div>
